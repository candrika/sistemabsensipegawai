...
function PersonIcon({ className }: { className?: string }) {
  return (
    <svg viewBox="0 0 48 56" fill="currentColor" className={className}>
...
const EMPLOYEE_STAT_CARDS = [
  { key: "total",     label: "DATABASE",    accent: "#2563eb", bg: "from-slate-50 to-blue-50/40",   border: "border-blue-100",   text: "text-blue-700",  num: "text-slate-800" },
  ...
  { key: "organik",   label: "ORGANIK",     accent: "#7c3aed", bg: "from-slate-50 to-violet-50/40", border: "border-violet-100", text: "text-violet-700",num: "text-slate-800" },
  { key: "tkjp",      label: "TKJP",        accent: "#0891b2", bg: "from-slate-50 to-cyan-50/40",   border: "border-cyan-100",   text: "text-cyan-700",  num: "text-slate-800" },
  { key: "security",  label: "SECURITY",    accent: "#ea580c", bg: "from-slate-50 to-orange-50/40", border: "border-orange-100", text: "text-orange-700",num: "text-slate-800" },
  { key: "mitra_kerja",label:"MITRA KERJA", accent: "#0d9488", bg: "from-slate-50 to-teal-50/40",   border: "border-teal-100",   text: "text-teal-700",  num: "text-slate-800" },
  { key: "mutasi",    label: "MUTASI",      accent: "#b45309", bg: "from-slate-50 to-amber-50/40",  border: "border-amber-100",  text: "text-amber-700", num: "text-slate-800" },
] as const;
...
export default function Dashboard() {
  ...
  return (
    <div className="space-y-10">
      ...
      <section>
        <h2 className="text-base font-bold text-muted-foreground uppercase tracking-widest mb-5">Status Kepegawaian</h2>
        <div className="grid gap-4 grid-cols-2 sm:grid-cols-3 lg:grid-cols-7">
          {EMPLOYEE_STAT_CARDS.map((card) => {
            ...
            return (
              <div
                key={card.key}
                className={`relative overflow-hidden rounded-2xl border shadow-sm hover:shadow-lg transition-all duration-300 hover:-translate-y-0.5 bg-gradient-to-br ${card.bg} ${card.border}`}
              >
                ...
                <div className="relative p-4 flex flex-col items-center text-center gap-3">
                  ...
                  <div className="flex items-center justify-center gap-2 mt-1">
                    <PersonIcon
                      className={`h-9 w-9 flex-shrink-0 drop-shadow-sm ${card.highlight ? "text-white/90" : ""}`}
